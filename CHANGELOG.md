...
### Fixed
- **Arrays**
  - `clean()` now honours `CleanFlag::RETURN_NULL` on an already empty input. An early return sitting in front of the flag test made `clean( [] , CleanFlag::NORMALIZE )` yield `[]` where `clean( [ 'a' => null ] , CleanFlag::NORMALIZE )` yielded `null` — two inputs meaning "no usable data", two return types, the most trivial one getting the exception. It also punched a hole through the validation pattern the `clean()` PHPDoc recommends : `if ( $cleaned === null ) throw ...` rejected a form full of blanks but accepted a wholly absent one. The early return is gone rather than patched, so the return contract keeps a single decision point. Flag validation still runs first, so an invalid mask on an empty input still throws. `normalize()` is unaffected : it already compensated for this on its own side.
  - `clean()` no longer injects a `null` into its own output. `CleanFlag::RETURN_NULL` was propagated to the recursive call, so with `RECURSIVE` set a nested array that cleaned to empty came back as `null` instead of `[]` and was stored as such — a `null` that `CleanFlag::NULLS` could not catch, since the value had already gone down the array branch. The flag is now stripped before recursing : it describes the contract of the outermost call only. Trigger window was `RETURN_NULL | RECURSIVE` **without** `EMPTY_ARR`, which is why `CleanFlag::DEFAULT` and `CleanFlag::NORMALIZE` were never affected — `EMPTY_ARR` discarded the `null` before it surfaced. Side benefit : a sub-array that was already empty on input and one that becomes empty while cleaning now yield the same `[]`, where the former gave `[]` and the latter `null`.
