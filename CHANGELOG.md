...
- **Date**
  ...
  - Add the `oihana\core\date\startOfDay()` and `endOfDay()` functions : the first and last representable instant of the day containing a `DateTimeInterface`, returned as a new `DateTimeImmutable` (`00:00:00.000000` / `23:59:59.999999`, microseconds included). The source is never modified and its timezone is preserved.
  - Add the `oihana\core\date\startOfMonth()` and `endOfMonth()` functions : same, for the month — the 1st at `00:00:00.000000`, the last day (leap years handled) at `23:59:59.999999`.
  - Add the `oihana\core\date\startOfYear()` and `endOfYear()` functions : same, for the year — January 1st / December 31st.
  - Add the `oihana\core\date\startOfWeek()` and `endOfWeek()` functions : same, for the week. `$firstDayOfWeek` uses the ISO-8601 numbering of `DateTime::format( 'N' )` — `1` (Monday, default) to `7` (Sunday, the US/Canada convention) — and is validated, an out-of-range value throwing `InvalidArgumentException`. `endOfWeek()` is `addDays( startOfWeek( ... ) , 6 )`, so the two always describe the same seven-day span regardless of which day of that span `$date` falls on.
  - Second batch ("boundaries") of the `date/` toolkit planned in `ROADMAP.md`.
  - Add the `oihana\core\date\isLeapYear()` function : pure arithmetic (the standard `%4==0 && (%100!=0 || %400==0)` proleptic-Gregorian rule), defined for any integer year. Extracted from the internal helper `addMonths()` already used for its own day-clamping, which now delegates to it.
  - Add the `oihana\core\date\isSameDay()` function : whether two instants fall on the same calendar day. The same physical instant can be a different calendar date depending on the timezone it is read in (`2026-01-01 00:30 UTC` is still `2025-12-31` in `America/Los_Angeles`) ; both dates are converted to `$timezone` before comparing, defaulting to the timezone carried by the first date — the common, no-argument call answers "same day, as seen from the first date's own timezone". Pass `$timezone` explicitly to compare from a third, unrelated timezone instead.
  - Add the `oihana\core\date\isToday()` function : `isSameDay( $date , $now ?? new DateTimeImmutable() , $timezone )` — same timezone rule.
  - Add the `oihana\core\date\tomorrow()` and `yesterday()` functions : `startOfDay( addDays( $now , ±1 ) )` — always midnight on the next/previous calendar day, regardless of the time of day `$now` carries.
  - Add the `oihana\core\date\diffInDays()` function : the signed, calendar-aware total number of days between two instants. `DateTimeInterface::diff()` is a trap for this — its `DateInterval::$d` is only the day-of-month component of a `y`/`m`/`d`/`h`/`i`/`s` breakdown, reset every month, so over a 34-day gap it reads `3`, not `34`. `diffInDays()` reads `$days` (the true total) instead, signed via `$invert` : positive when the second date is later, negative when earlier.
  - Add the `oihana\core\date\diffInHours()` function : the signed, elapsed-time (not calendar-aware) number of complete hours between two instants, computed from the Unix timestamps. Deliberately disagrees with `diffInDays()` across a DST transition — `2026-03-28 00:00` to `2026-03-30 00:00 Europe/Paris` is 2 calendar days but only 47 real hours, the spring-forward gap on `2026-03-29` having shortened that day by one hour.
...
### Changed
- **Tests / Tooling**
  - Migrate `phpunit.xml` to the PHPUnit 13.3 schema. `composer.lock` is gitignored and the constraint is `phpunit/phpunit: ^13`, so a plain `composer install` moved the suite from 13.2 to 13.3, where the runner started reporting two issues and exiting with code `1` on an otherwise green tree : the `cacheResult` attribute is deprecated in favour of `recordTestRunHistory`, and `executionOrder="depends,defects"` warns that it cannot order by defects while that history is disabled. The two settings had in fact always contradicted each other — `defects` ordering needs the cache `cacheResult="false"` was turning off, so it was inert — and the deliberate no-cache choice is kept : `recordTestRunHistory="false"`, `executionOrder="depends"`. No test behaviour change.
- **Static analysis**
...
### Fixed
- **Date**
  - `oihana\core\date\formatDateTime()` (and `now()`, which delegates to it) appended the literal `Z` of the default format **without converting the moment to UTC**, so an offset-bearing `2025-07-20T09:30:00+02:00` came back labelled `2025-07-20T09:30:00.000Z` — two hours off — and `now( 'Europe/Paris' )` stamped the Paris wall clock as if it were UTC, contradicting its own PHPDoc ("always converted to UTC"). The moment is now converted to UTC whenever the format carries a **literal** `Z`, the ISO-8601 designator for UTC ; a format without one (`'Y-m-d\TH:i:s'`, `'d/m/Y H:i'`) keeps rendering in the timezone the moment was parsed in, as before, and so does one carrying a real offset token (`P`, `O`, `T`), that token already saying which timezone it is. An escaped backslash before the `Z` (`'\\Z'`) is correctly read as the native offset-in-seconds token, not as a Zulu designator.
    - **Behaviour change** for the one combination that was wrong : a non-UTC `$timezone` (or an offset-bearing input) together with a `Z`-suffixed format. Callers relying on the previous output were being served timestamps several hours off. Timestamps already persisted through that path are **not** retro-migrated.
  - Fix the `formatDateTime()` example of `README.md`, which passed the format string as the `$timezone` argument and would throw `DateInvalidTimeZoneException`.
