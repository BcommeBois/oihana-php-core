...
final class TarFile
{
     ...
     * @var string[]
     */
    public const array TAR_EXTENSIONS = [ FileExtension::GZ , FileExtension::TAR , FileExtension::TGZ , FileExtension::TAR_GZ ] ;

    /**
     * Heuristic: validate a file is a tar or gzip‑compressed tar archive.
     * @param string $file The file to evaluates.
     * @return bool True if the file is a tar file, false otherwise.
     ...
     */
    public function assertFile( string $file ): bool
    {
        if ( !is_file( $file ) )
        {
            throw new FileException( sprintf('The file "%s" does not exist.' , $file ) ) ;
        }
        ...
        if ( !str_contains($mimeType, 'x-tar') && !str_contains( $mimeType, 'gzip' ) )
        {
    ...
    public function create( string $directory, string $extension = FileExtension::TAR_GZ ): string
    {
        if ( !is_dir( $directory ) )
        {
            throw new DirectoryException( sprintf('Creates a tar file failed, the passed-in directory %s does not exist.' , json_encode( $directory , JSON_UNESCAPED_SLASHES ) ) ) ;
        }
    ...
    public function extract( string $tarFile , string $directory ): true
    {
        ...
        {
            throw new RuntimeException( sprintf('Extract tar file failed, impossible to extract the tar file %s to directory %s.' , json_encode( $tarFile , JSON_UNESCAPED_SLASHES ) , json_encode( $directory , JSON_UNESCAPED_SLASHES ) ) , $status ) ;
        }
    }
}
